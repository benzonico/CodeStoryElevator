...
public class ElevatorEngineTest {
	...
	@Test
	public void initServices_should_listen_on_specified_port() throws ClientProtocolException, IOException {
		int port = 8000;
		ElevatorEngine.initServices(port);
		String[] domains = {"go","call","reset","userHasEntered","userHasExited","nextCommand"};
		for (int i = 0; i < domains.length; i++) {
			HttpGet get = new HttpGet("http://localhost:"+port+"/"+domains[i]);
